...
static const TYPE read_struct(const IDevice* device, const CoreCoord core, uint32_t l1_addr) {
    const TYPE status = {0};

    auto raw_status = MetalContext::instance().get_cluster().read_core(
        device->id(), device->worker_core_from_logical_core(core), l1_addr, sizeof status);

    memcpy((void*)&status, raw_status.data(), sizeof status);

    return status;
}

template <typename TYPE>
static void write_struct(const IDevice* device, const CoreCoord core, uint32_t l1_addr, const TYPE* in) {
    MetalContext::instance().get_cluster().write_core(
        device->id(),
        device->worker_core_from_logical_core(core),
        vector<uint32_t>((uint32_t*)in, (uint32_t*)in + (sizeof *in + sizeof(uint32_t) - 1) / sizeof(uint32_t)),
        l1_addr);
}

template <typename STRUCT>
static uint64_t get_completed_jobs_total(
    const IDevice* device, const vector<vector<DramWorkItem>>& jobs_per_core, span<STRUCT> per_core) {
    /* ======================== */
    uint64_t completed = 0;

    for (size_t i = 0; i < per_core.size(); i++) {
        const auto status = read_struct<CoreProgressStatus>(device, per_core[i].core, per_core[i].status_l1_addr);
        completed += std::min<uint64_t>(status.jobs_completed, jobs_per_core[i].size());
    }
...
[[maybe_unused]]
DramMultiInstanceSummary run_dram_persistent_jobs_test_verbose_parallel(
    tt::tt_metal::MeshDispatchFixture* fixture,
    const std::shared_ptr<tt::tt_metal::distributed::MeshDevice>& mesh_device,
    const std::vector<CoreCoord>& worker_cores,
    const std::vector<std::vector<DramWorkItem>>& jobs_per_core,
    ...
    for (const auto& core : worker_cores) {
        ...
        write_struct<DramJobQueueCtrl>(device, core, r.queue_ctrl_l1_addr, &ctrl);

        CoreProgressStatus status{};
        dram_progress_status_init(status);

        write_struct<CoreProgressStatus>(device, core, r.status_l1_addr, &status);

        const DramBaseResult zero_results = {0};
        write_struct<DramBaseResult>(device, core, r.result_ring_l1_addr, &zero_results);
        ...
        write_core_u32(device, core, r.wake_flag_l1_addr, 0);
    ...
    for (size_t core_idx = 0; core_idx < per_core.size(); core_idx++) {
        ...
        write_struct<DramJobQueueCtrl>(device, r.core, r.queue_ctrl_l1_addr, &ctrl);
        write_core_u32(device, r.core, r.wake_flag_l1_addr, 1);
    }
    ...
    std::thread refill_thread([&]() {
        ...
        while (!done) {
            ...
            if ((monitor_now - global_last_monitor_print_time) >= kMonitorPrintInterval) {
                ...
                for (auto& rr : per_core) {
                    const size_t job_size = jobs_per_core[&rr - per_core.data()].size();
                    const auto status = read_struct<CoreProgressStatus>(device, rr.core, rr.status_l1_addr);

                    completed_jobs_total += std::min<uint64_t>(status.jobs_completed, job_size);

                    const bool core_done = status.jobs_completed >= job_size;
                    const bool core_progressing = (status.heartbeat_tick != rr.prev_heartbeat_tick) ||
                                                  (status.jobs_completed != rr.prev_jobs_completed);
            ...
            for (size_t core_idx = 0; core_idx < per_core.size(); core_idx++) {
                ...
                const auto status = read_struct<CoreProgressStatus>(device, r.core, r.status_l1_addr);
                const auto now = std::chrono::steady_clock::now();
                ...
                while (r.jobs_observed_done < r.jobs_enqueued) {
                    ...
                    while (!result_ready) {
                        ...
                        if ((monitor_now - global_last_monitor_print_time) >= kMonitorPrintInterval) {
                            const uint64_t arc_tick = read_arc_global_tick(device);
                            const uint64_t completed_jobs_total =
                                get_completed_jobs_total<PerCorePersistentResources>(device, jobs_per_core, per_core);
                        ...
                        result_copy = read_struct<DramBaseResult>(
                            device, r.core, r.result_ring_l1_addr + done_slot * sizeof result_copy);
                        ...
                        const auto status_poll = read_struct<CoreProgressStatus>(device, r.core, r.status_l1_addr);
                ...
                if (!g_stop_requested.load() && r.jobs_enqueued < core_jobs.size()) {
                    ...
                    while (r.jobs_enqueued < core_jobs.size()) {
                        ...
                        const DramWorkItem* next_job = &core_jobs[r.jobs_enqueued];
                        write_struct<DramWorkItem>(
                            device, r.core, r.queue_jobs_l1_addr + slot * sizeof *next_job, next_job);
